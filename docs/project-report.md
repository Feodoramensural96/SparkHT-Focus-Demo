...
每批最多四图、按时间排序，`temperature=0`、`max_tokens=192`、超时 30 秒。响应必须严格匹配 Pydantic schema；第一次失败后只做一次 JSON 修复，再失败记录 `analysis_failed`。
...
尚未完成的 Step3 和真机数字明确保留为待实测。最终报告只在完成真实单图/四图、语音抢占、两轮 90 秒流程后填写这些栏位。
