...
本机真实 FP8 8040 已完成单图 64-token 结构化输出，以及最终配置的四图 192-token 三轮基准；四图 P50/P95 为 9.564/11.212 秒。8 帧编排闭环以 9.744/9.874 秒完成两批并持久化 8 个观察。测试图只证明协议与部署，不冒充机器人场景准确率。机器人相机 P95、首音、语音抢占和两轮 90 秒数字继续保留为待真机实测。
